...
public abstract class OcrMath extends OcrCore {
	...
	protected List<RasterRegion> processRegions(List<RasterRegion> regions) {
		...
		for (RasterRegion region : regions) {
			try {
				mathTree.addChild(new Symbol(region.tag.toString(), mathTree.getRoot(), region));
			} catch (NullPointerException e) {
				throw new NullPointerException("@See OcrBasic#findRegions hardcoded string (temporary)");
			}
		}
	...
	public void findAbove(List<RasterRegion> regions, List<MathTreeNode> previous, Expresion parent) {
	...
	@Deprecated
	public void findAboveDeprecated(List<RasterRegion> regions, Expresion parent) {
		List<RasterRegion> upperAboveRegions = new ArrayList<RasterRegion>();
		...
		for (int i = 0; i < regions.size(); i++) {
			for (int j = i + 1; j < regions.size(); j++) {
				if (isUpperRight(regions.get(i), regions.get(j))) {
					upperAboveRegions.add(regions.get(j));
					upperAboveCurrent.add(new Symbol(regions.get(j).tag.toString(), child, regions.get(j)));
					System.out.println(regions.get(j).tag + " is above");
				} else if (regions.get(i) != regions.get(j)) {
					break;
				}
			...
			if (upperAboveCurrent.size() > 0) {
				...
				System.out.println();
				if (parent == null) {
					Symbol temp = new Symbol(regions.get(i).tag.toString(), null, regions.get(i));
					child = new Expresion(new ArrayList<MathTreeNode>(upperAboveCurrent), false, temp, ExpresionType.UPPERRIGHT);
				} else {
					child = new Expresion(new ArrayList<MathTreeNode>(upperAboveCurrent), false, parent, ExpresionType.UPPERRIGHT);
				}
				mathTree.addChild(child);
				regions.removeAll(upperAboveRegions);
				if (parent != null) {
					parent.removeChildren(upperAboveCurrent);
				}
				findAboveDeprecated(upperAboveRegions, child);
			}
